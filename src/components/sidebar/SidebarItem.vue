...
const isActive = computed(() => {
  return (
    route.meta?.sidebarKey && route.meta.sidebarKey === props.item.sidebarKey
  );
});
</script>
    ...
    <div class="accordion group/accordion flex flex-col gap-1 w-full">
      ...
      >
        <div class="relative flex size-6 shrink-0">
          <img
            :src="item.iconActive"
            class="absolute flex size-6 shrink-0 opacity-0 transition-setup"
            alt="icon"
          />
          <img
            :src="item.iconInActive"
            class="absolute flex size-6 shrink-0 opacity-100 transition-setup"
            alt="icon"
          />
        </div>
        <span
          class="text-left leading-5 text-desa-secondary flex flex-1 transition-setup"
        >
          {{ item.label }}
        </span>
        <div class="relative flex size-6 shrink-0">
