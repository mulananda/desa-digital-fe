...
const { loading, headOfFamilyData } = storeToRefs(headOfFamilyStore);
...
// ✅ FIX: onMounted HARUS function
onMounted(() => {
  if (headOfFamilyId.value) {
    fetchHeadOfFamily(headOfFamilyId.value);
  }
});

// ✅ Jika pindah ID tanpa reload halaman
watch(
  headOfFamilyId,
  (id) => {
    if (id) fetchHeadOfFamily(id);
  },
  { immediate: false }
);
</script>
    ...
    <div class="flex flex-col w-[calc(525/1000*100%)] shrink-0 gap-[14px]">
      ...
      >
        ...
        <div class="flex items-center gap-4">
          ...
          >
            <img
              v-if="headOfFamilyData?.profile_picture"
